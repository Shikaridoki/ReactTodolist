    ...
    return (
        <>
            <ul className="content__menu">
                {
                    user.categories.find((item)=> item.categoryName === statusContent).tasks.length === 0 ? <span>Задачи отсутствуют</span> :
                        user.categories.find((item)=> item.categoryName === statusContent).tasks.map((item)=>(
                            <li className='content__item' key={item.id}>
                                ...
                                <span className="content__item-del" onClick={()=>delTask(item.id,statusContent)}>
                                    <svg width="11" height="11" viewBox="0 0 11 11"  fill="#E3E3E3" xmlns="http://www.w3.org/2000/svg">
