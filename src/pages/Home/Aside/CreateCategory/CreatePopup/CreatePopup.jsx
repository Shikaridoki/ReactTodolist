...
const CreatePopup = ({active,setActive}) => {
    ...
    const {checkCategory} =useContext(CustomContext)


    return (

        <div className="aside__popup" style={{display:active ? 'block' : 'none'}}>
            <input value={category} onChange={(e) =>setCategory(e.target.value)} className="aside__field" placeholder='Название папки'/>
            <div className="aside__colors">
                {dataColors.map((item) => (
                    <span className="aside__col" onClick={() => setColor(item)} key={item} style={{background: item, border : color === item ? '4px solid black' : 'none'}}></span>
                ))}
            </div>
            <button className="aside__add" type='button' onClick={()=>checkCategory(category,color,setActive,setCategory)}>Добавить</button>
