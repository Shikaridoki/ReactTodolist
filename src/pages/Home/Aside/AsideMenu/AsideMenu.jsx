    ...
    return (
        ...
        <ul className='aside__menu'>
            {user.categories.map((item)=>(
                <li className={`aside__item ${status === item.categoryName ? 'active' : ''}`} key={item.id} onClick={()=> setStatus(item.categoryName)}>
                    <span className='aside__color' style={{background : item.color}}></span>
